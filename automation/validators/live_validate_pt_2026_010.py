...
OUTPUT = REPO_ROOT / 'detections' / 'validation' / 'live' / 'VAL-2026-010-PT-2026-010.json'
QUERY = REPO_ROOT / 'detections' / 'generated' / 'splunk' / 'live' / 'suspicious_rundll32_proxy_execution.spl'
VM = 130
...
def marker_state(marker: str) -> str:
    return guest_ps(f"Get-Item '{marker}' -ErrorAction SilentlyContinue | Select-Object FullName,Length,LastWriteTime | Format-List")
...
def main() -> int:
    ...
    original = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_010_positive_url_fileprotocolhandler.ps1').read_text(encoding='utf-8')
    variant = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_010_positive_pcwutl_launchapplication.ps1').read_text(encoding='utf-8')
    negatives = [
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_010_negative_rundll32_control.ps1').read_text(encoding='utf-8'),
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_010_negative_rundll32_noop.ps1').read_text(encoding='utf-8'),
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_010_negative_controlpanel.ps1').read_text(encoding='utf-8'),
    ]
    cleanup = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_010_cleanup.ps1').read_text(encoding='utf-8')
    result = {
        'scenario_id': 'PT-2026-010',
        'validation_run_id': 'VAL-2026-010',
        'technique_id': 'T1218.011',
        'original': run_test(original, 'C:\\Windows\\Temp\\pt-2026-010-url.txt'),
        'variant': run_test(variant, 'C:\\Windows\\Temp\\pt-2026-010-pcwu.txt'),
        'negatives': [
            run_test(negatives[0], 'C:\\Windows\\Temp\\pt-2026-010-url.txt', 3),
            run_test(negatives[1], 'C:\\Windows\\Temp\\pt-2026-010-url.txt', 3),
            run_test(negatives[2], 'C:\\Windows\\Temp\\pt-2026-010-url.txt', 3),
        ],
        'cleanup': {
