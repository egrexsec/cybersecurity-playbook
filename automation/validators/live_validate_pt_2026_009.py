...
OUTPUT = REPO_ROOT / 'detections' / 'validation' / 'live' / 'VAL-2026-009-PT-2026-009.json'
QUERY = REPO_ROOT / 'detections' / 'generated' / 'splunk' / 'live' / 'suspicious_powershell_profile_persistence.spl'
VM = 130
...
def profile_state() -> str:
    script = "$PROFILE | Out-Host; Test-Path $PROFILE | Out-Host; if (Test-Path $PROFILE) { Get-Content $PROFILE -Raw | Out-Host }"
    return guest_ps(script)


def run_test(script: str, wait_seconds: int = 5) -> dict[str, Any]:
    start = utc_now()
...
def main() -> int:
    ...
    original = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_009_positive_addcontent_profile.ps1').read_text(encoding='utf-8')
    variant = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_009_positive_append_profile_variant.ps1').read_text(encoding='utf-8')
    negatives = [
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_009_negative_testpath_profile.ps1').read_text(encoding='utf-8'),
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_009_negative_getcontent_profile.ps1').read_text(encoding='utf-8'),
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_009_negative_write_benign_profile_note.ps1').read_text(encoding='utf-8'),
    ]
    cleanup = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_009_cleanup.ps1').read_text(encoding='utf-8')
    result = {
        'scenario_id': 'PT-2026-009',
        'validation_run_id': 'VAL-2026-009',
        'technique_id': 'T1546.013',
        'original': run_test(original),
        'variant': run_test(variant),
        'negatives': [run_test(negatives[0], 3), run_test(negatives[1], 3), run_test(negatives[2], 3)],
        'cleanup': {
