...
OUTPUT = REPO_ROOT / 'detections' / 'validation' / 'live' / 'VAL-2026-005-PT-2026-005.json'
QUERY = REPO_ROOT / 'detections' / 'generated' / 'splunk' / 'live' / 'suspicious_windows_service_creation.spl'
VM = 130
...
def local_system_query(service_name: str) -> str:
    return (
        "Get-WinEvent -LogName 'System' -MaxEvents 30 | "
        f"Where-Object {{ $_.Message -match [regex]::Escape('{service_name}') }} | "
        "Select-Object TimeCreated,Id,ProviderName,Message | ConvertTo-Json -Depth 4"
    )


def run_test(script: str, service_name: str | None = None, wait_seconds: int = 5) -> dict[str, Any]:
    start = utc_now()
    ...
    local_system_logs = guest_ps(local_system_query(service_name)) if service_name else ''
    return {
        'start_time_utc': iso_z(start),
        'end_time_utc': iso_z(end),
        'exec_output': exec_output,
        'detection_results': detection_results,
        'local_system_logs': local_system_logs,
...
def main() -> int:
    ...
    original = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_005_positive_sc_create.ps1').read_text(encoding='utf-8')
    variant = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_005_positive_new_service.ps1').read_text(encoding='utf-8')
    negatives = [
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_005_negative_sc_qc.ps1').read_text(encoding='utf-8'),
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_005_negative_get_service.ps1').read_text(encoding='utf-8'),
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_005_negative_benign_service_create.ps1').read_text(encoding='utf-8'),
    ]
    cleanup = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_005_cleanup.ps1').read_text(encoding='utf-8')

    result = {
        'scenario_id': 'PT-2026-005',
        'validation_run_id': 'VAL-2026-005',
        'technique_id': 'T1543.003',
        'original': run_test(original, 'PT-2026-005-Original'),
        'variant': run_test(variant, 'PT-2026-005-Variant'),
        'negatives': [
            run_test(negatives[0], None, 3),
            run_test(negatives[1], None, 3),
            run_test(negatives[2], 'PT-2026-005-Benign', 3),
        ],
        'cleanup': {
