...
OUTPUT = REPO_ROOT / 'detections' / 'validation' / 'live' / 'VAL-2026-008-PT-2026-008.json'
QUERY = REPO_ROOT / 'detections' / 'generated' / 'splunk' / 'live' / 'suspicious_bits_job_creation.spl'
VM = 130
...
def local_bits_query(marker: str) -> str:
    return (
        "Get-WinEvent -LogName 'Microsoft-Windows-Bits-Client/Operational' -MaxEvents 30 | "
        f"Where-Object {{ $_.Message -match [regex]::Escape('{marker}') }} | "
        "Select-Object TimeCreated,Id,Message | ConvertTo-Json -Depth 4"
    )


def run_test(script: str, marker: str | None = None, wait_seconds: int = 6) -> dict[str, Any]:
    start = utc_now()
    ...
    local_bits_logs = guest_ps(local_bits_query(marker)) if marker else ''
    return {
        'start_time_utc': iso_z(start),
        'end_time_utc': iso_z(end),
        'exec_output': exec_output,
        'detection_results': detection_results,
        'local_bits_logs': local_bits_logs,
...
def main() -> int:
    ...
    original = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_008_positive_bitsadmin_transfer.ps1').read_text(encoding='utf-8')
    variant = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_008_positive_startbitstransfer.ps1').read_text(encoding='utf-8')
    negatives = [
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_008_negative_get_bits.ps1').read_text(encoding='utf-8'),
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_008_negative_bits_service.ps1').read_text(encoding='utf-8'),
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_008_negative_bitsadmin_list.ps1').read_text(encoding='utf-8'),
    ]
    cleanup = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_008_cleanup.ps1').read_text(encoding='utf-8')
    result = {
        'scenario_id': 'PT-2026-008',
        'validation_run_id': 'VAL-2026-008',
        'technique_id': 'T1197',
        'original': run_test(original, 'PT-2026-008-Original'),
        'variant': run_test(variant, 'BITS Transfer'),
        'negatives': [
