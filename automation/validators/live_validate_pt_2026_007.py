...
OUTPUT = REPO_ROOT / 'detections' / 'validation' / 'live' / 'VAL-2026-007-PT-2026-007.json'
QUERY = REPO_ROOT / 'detections' / 'generated' / 'splunk' / 'live' / 'suspicious_logon_script_registry_persistence.spl'
VM = 130
...
def local_hive_query(name: str) -> str:
    return (
        "cmd.exe /c \"reg unload HKU\\PT2026007 >nul 2>&1\"; "
        "cmd.exe /c \"reg load HKU\\PT2026007 C:\\Users\\mell0wx\\NTUSER.DAT\" | Out-Host; "
        f"Get-ItemProperty -Path 'Registry::HKEY_USERS\\PT2026007\\Environment' -Name '{name}' -ErrorAction SilentlyContinue | ConvertTo-Json -Compress | Out-Host; "
        "cmd.exe /c \"reg unload HKU\\PT2026007\" | Out-Host"
    )


def run_test(script: str, name: str | None = None, wait_seconds: int = 5) -> dict[str, Any]:
    start = utc_now()
    ...
    hive_state = guest_ps(local_hive_query(name)) if name else ''
    return {
        'start_time_utc': iso_z(start),
        'end_time_utc': iso_z(end),
        'exec_output': exec_output,
        'detection_results': detection_results,
        'local_hive_state': hive_state,
...
def main() -> int:
    ...
    original = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_007_positive_reg_logonscript.ps1').read_text(encoding='utf-8')
    variant = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_007_positive_powershell_logonscript.ps1').read_text(encoding='utf-8')
    negatives = [
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_007_negative_query_env.ps1').read_text(encoding='utf-8'),
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_007_negative_reg_query.ps1').read_text(encoding='utf-8'),
        (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_007_negative_benign_path_value.ps1').read_text(encoding='utf-8'),
    ]
    cleanup = (REPO_ROOT / 'automation' / 'execution' / 'pt_2026_007_cleanup.ps1').read_text(encoding='utf-8')
    result = {
        'scenario_id': 'PT-2026-007',
        'validation_run_id': 'VAL-2026-007',
        'technique_id': 'T1037.001',
        'original': run_test(original, 'UserInitMprLogonScript'),
        'variant': run_test(variant, 'UserInitMprLogonScript'),
        'negatives': [
